<template>
  <div class="py-8">
    <div class="container mx-auto px-4">
      <h2 class="text-2xl font-bold mb-8 text-amber-800">Tu Carrito de Compras</h2>
      ...
      <div v-else-if="error" class="text-center py-10 bg-red-50 border border-red-200 rounded-lg p-6">
      ...
      <div v-else-if="!cart || !cart.items || cart.items.length === 0" class="text-center py-10 bg-amber-50 border border-amber-200 rounded-lg p-6">
        <p class="text-amber-800 text-lg mb-4">Tu carrito está vacío</p>
        <p class="text-amber-700 mb-6">Agrega algunos productos deliciosos para comenzar tu pedido.</p>
        <router-link 
          to="/products" 
      ...
      <div v-else class="flex flex-col md:flex-row gap-8">
        <div class="md:w-2/3">
          <div class="bg-white rounded-lg shadow-md p-6">
            <div class="space-y-4">
              <div v-for="item in cart.items" :key="item.product._id" 
                ...
                <div class="flex items-center gap-4 mb-2 sm:mb-0">
                  <img 
                    :src="item.product.image || 'https://avatars.mds.yandex.net/get-altay/13451497/2a0000018ed9af680cbea3c965bebbc1a280/XXXL'" 
                    :alt="item.product.name"
                    class="w-16 h-16 object-cover rounded"
                  />
                  <div>
                    <h3 class="font-medium text-amber-800">{{ item.product.name }}</h3>
                    <p class="text-amber-600 text-sm">{{ item.product.category?.name || 'Producto' }}</p>
                    <p class="font-bold text-amber-900">${{ formatPrice(item.product.price) }}</p>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                  ...
                  <div class="text-right min-w-[80px]">
                    <p class="font-bold text-amber-900">${{ formatPrice(item.quantity * item.product.price) }}</p>
        ...
        <div class="md:w-1/3">
          <div class="bg-amber-50 p-6 rounded-lg shadow-md sticky top-4">
            <h3 class="text-xl font-bold mb-4 text-amber-800">Resumen del pedido</h3>
